...
def bigrams_count_to_probabilities(
    ...
    row_sums = smoothed_bigrams.sum(dim=1,keepdim=True)
    bigram_probs = smoothed_bigrams / row_sums
...
def calculate_log_likelihood(
    ...
    for i in range(len(processed_word)-1):
        first_index = char_to_index[processed_word[i]]
        second_index = char_to_index[processed_word[i+1]]
        prob = bigram_probabilities[first_index, second_index]
